...
function at(texture, lat, lon) {
  ...
  return Array.from(texture.image.data.subarray(offset, offset + 4), texture.type === THREE.FloatType ? v=>v : THREE.DataUtils.fromHalfFloat);
}
...
for (const name of names) {
  ...
  assert.equal(t.type, name === 'aridity' ? THREE.FloatType : THREE.HalfFloatType); assert.equal(t.minFilter, name === 'aridity' ? THREE.NearestFilter : THREE.LinearFilter);
  assert.equal(t.magFilter, t.minFilter); assert.equal(t.flipY, false);
  assert.equal(t.generateMipmaps, false); assert.equal(t.colorSpace, THREE.NoColorSpace);
  const data = t.image.data;
  for (let i = 0; i < data.length; i += 4) {
    assert.ok(data[i+3] === 0 || data[i+3] === (name === 'aridity' ? 1 : 0x3c00));
    if (!data[i+3]) assert.ok(data[i] === 0 && data[i+1] === 0 && data[i+2] === 0);
    else for (let c = 0; c < 3; c++) assert.ok(Number.isFinite(name === 'aridity' ? data[i+c] : THREE.DataUtils.fromHalfFloat(data[i+c])));
  }
}
...
for (let row = 0; row < 720*360; row++) {
  ...
  assert.equal(encoded[offset+3], valid ? 1 : 0);
  if (!valid) continue;
  validDM++;
  if (dm.some(value => value > 60)) cappedDM++;
  if (dm.some(value => value > 65504)) previouslyOverflowedDM++;
  dm.forEach((value, i) => close(encoded[offset+i], value));
}
assert.ok(previouslyOverflowedDM > 0);
assert.equal(textures.aridity.userData.scientific.displayCap, null);
assert.equal(textures.aridity.userData.scientific.displayCappedCells, 0);
  ...
  try {
    ...
    const result = await page.evaluate(async () => {
      ...
      for (const texture of Object.values(textures)) {
        ...
        const candidates = [{ index: valid }, { index: 720*359+360 }, { index: 720*180+180 }, { index: edge, boundary: texture.minFilter === THREE.LinearFilter }];
        for (const { index, boundary } of candidates) {
          const decode = texture.type === THREE.FloatType ? v=>v : THREE.DataUtils.fromHalfFloat;
          const expected = Array.from(data.subarray(index*4, index*4+4), decode);
          if (boundary) for (let c = 0; c < 4; c++) expected[c] = (expected[c] + decode(data[(index+1)*4+c]))/2;
