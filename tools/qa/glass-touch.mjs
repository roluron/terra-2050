...
for(const [name,engine,options] of [
 ...
 try{
  await page.goto((process.env.URL0||'http://127.0.0.1:8088/')+'?lang=fr');await page.locator('#language-dialog[open]').waitFor();await glass('.language-content');await shot('language');await tap('#language-continue');
  ...
  if(mobile){await target('#map-toggle');await tap('#map-toggle');}
  ...
  await glass('#map-inspector');await target('[data-map-mode="value"]');await shot('map');await tap('#map-method summary');await shot('sources');await tap('#map-method summary');
