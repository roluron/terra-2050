...
try{
 ...
 const shot=(key,t,p)=>{for(const c of Object.values(CALQUES))c.uniforme.value=0;if(key)CALQUES[key].uniforme.value=1;uniformsGlobe.uIndice.value=0;uniformsGlobe.uTemps.value=t;uniformsGlobe.uProgression.value=p;moteur.setRenderTarget(target);moteur.render(scene,camera);const pixels=new Uint8Array(w*h*4);moteur.readRenderTargetPixels(target,0,0,w,h,pixels);moteur.setRenderTarget(null);return pixels;};
