...
try{
 ...
 await page.route('**/*',async route=>{
  ...
  const response=await route.fetch();let html=await response.text();html=html.replace('</script>\n</body>',`globalThis.__motionClock=()=>[uniformsGlobe.uTemps.value,feuxUniforms.uTemps.value];globalThis.__motionProbe=()=>{
 ...
 const animation={};for(const key of Object.keys(CALQUES))animation[key]=diff(shot(key,0,1),shot(key,1.7,1));const fireYear=diff(shot('feux',0,0),shot('feux',0,1)),riverYear=diff(shot('fleuves',0,0),shot('fleuves',0,1));const visibility=scene.children.map(o=>o.visible);scene.children.forEach(o=>o.visible=o.material===matAtmo);matAtmo.uniforms.uProgression.value=0;const heat2026=diff(shot('chaleur',0,0),shot('chaleur',1.7,0));matAtmo.uniforms.uProgression.value=1;const heat2050=diff(shot('chaleur',0,1),shot('chaleur',1.7,1));scene.children.forEach((o,i)=>o.visible=visibility[i]);return{animation,fireYear,riverYear,heat2026,heat2050,width:w,height:h};};\n</script>\n</body>`);
  await route.fulfill({response,body:html});
 });
 await page.goto(process.env.URL0||'http://localhost:8087/');await enter(page);await page.waitForTimeout(4500);
 ...
 assert.ok(result.pixels.fireYear.changed>100);assert.equal(result.pixels.riverYear.changed,0);assert.deepEqual(errors,[]);await page.emulateMedia({reducedMotion:'reduce'});await page.reload();await enter(page);await page.waitForTimeout(1200);result.reducedClock=await page.evaluate(()=>globalThis.__motionClock());assert.deepEqual(result.reducedClock,[0,0]);result.pass=true;
