...
try{
 ...
 await page.route('**/*',async route=>{
  ...
  const response=await route.fetch();let html=await response.text();html=html.replace('</script>\n</body>',`globalThis.__motionReady=()=>!!uniformsGlobe.uScientific_river.value.image?.data&&uniformsGlobe.uPopulation.value!==texVide;globalThis.__motionClock=()=>[uniformsGlobe.uTemps.value,fleuvesUniforms.uTemps.value];globalThis.__motionProbe=()=>{
 ...
 const animation={},years={};for(const key of Object.keys(CALQUES)){animation[key]=diff(shot(key,0,1),shot(key,1.7,1));years[key]=diff(shot(key,0,0),shot(key,0,1));}const fireBaseline=diff(shot('feux',0,0),shot(null,0,0));const fireYear=years.feux,riverYear=years.fleuves;
 groupeFleuves.visible=true;fleuvesUniforms.uCrue.value=.45;fleuvesUniforms.uTemps.value=0;const riverStart=shot('fleuves',0,1);fleuvesUniforms.uTemps.value=1.7;const riverLines=diff(riverStart,shot('fleuves',0,1));groupeFleuves.visible=false;
 const visibility=scene.children.map(o=>o.visible);scene.children.forEach(o=>o.visible=o.material===matAtmo);matAtmo.uniforms.uProgression.value=0;const heat2026=diff(shot('chaleur',0,0),shot('chaleur',1.7,0));matAtmo.uniforms.uProgression.value=1;const heat2050=diff(shot('chaleur',0,1),shot('chaleur',1.7,1));scene.children.forEach((o,i)=>o.visible=visibility[i]);return{animation,years,riverLines,fireBaseline,fireYear,riverYear,heat2026,heat2050,width:w,height:h};};\n</script>\n</body>`);
 ...
 for(const [k,v]of Object.entries(result.pixels.animation))assert.equal(v.changed,0,JSON.stringify({k,v}));
 for(const [k,v]of Object.entries(result.pixels.years))assert.ok(v.changed>0,JSON.stringify({k,v}));
 assert.ok(result.pixels.riverLines.changed>0,'River course lines animate independently of hazard colors');
 assert.equal(result.pixels.heat2026.changed,0);assert.ok(result.pixels.heat2050.changed>100);
 assert.ok(result.pixels.fireBaseline.changed>100);assert.ok(result.pixels.fireYear.changed>100);assert.ok(result.pixels.riverYear.changed>0, 'Paired river epochs must change the source-backed map');assert.deepEqual(errors,[]);await page.emulateMedia({reducedMotion:'reduce'});await page.reload();await enter(page);await page.waitForTimeout(1200);result.reducedClock=await page.evaluate(()=>globalThis.__motionClock());assert.deepEqual(result.reducedClock,[0,0]);result.pass=true;
